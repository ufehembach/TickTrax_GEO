...
fun formatDate4Recycler(date: Date,  locale: Locale = Locale.getDefault()): String {
    ...
    return when {
        DateUtils.isToday(date.time) -> {
            ...
            SimpleDateFormat("$timeFormat", locale).format(date)
        }
        DateUtils.isToday(date.time + DateUtils.DAY_IN_MILLIS) -> {
            // Yesterday: "yesterday" + time and day
            "Gestern, " + SimpleDateFormat("$timeFormat", locale).format(date)
        }
